...
const CHALLENGES = [
  { subject: 'Mathematics', type: 'mcq', difficulty: 2, xp_reward: 30, content: { question: 'If f(x) = 2x^2 - 3x + 1, what is f(3)?', options: { A: '10', B: '12', C: '14', D: '16' }, correct_answer: 'A', explanation: 'f(3) = 2(9) - 9 + 1 = 10', hints: ['Substitute x=3'] } },
  { subject: 'Mathematics', type: 'mcq', difficulty: 2, xp_reward: 30, content: { question: 'Solve for x: x^2 - 5x + 6 = 0', options: { A: 'x = 2 or x = 3', B: 'x = 1 or x = 6', C: 'x = -2 or x = -3', D: 'x = 3 or x = 4' }, correct_answer: 'A', explanation: 'Factor: (x-2)(x-3) = 0', hints: ['Find two numbers that multiply to 6'] } },
  ...
  { subject: 'Physical Sciences', type: 'mcq', difficulty: 2, xp_reward: 30, content: { question: 'According to Newton, what is F when m=2kg and a=5m/s^2?', options: { A: '10N', B: '7N', C: '3N', D: '25N' }, correct_answer: 'A', explanation: 'F = ma = 2 * 5 = 10N', hints: ['Use F = ma'] } },
  ...
  { subject: 'Life Sciences', type: 'mcq', difficulty: 2, xp_reward: 30, content: { question: 'What is the process by which plants make their food?', options: { A: 'Respiration', B: 'Photosynthesis', C: 'Fermentation', D: 'Transpiration' }, correct_answer: 'B', explanation: 'Photosynthesis converts CO2 to glucose', hints: ['Think about sunlight'] } },
  ...
  { subject: 'Business Studies', type: 'mcq', difficulty: 2, xp_reward: 30, content: { question: 'What is the main purpose of a business plan?', options: { A: 'Tax rebate', B: 'Guide operations', C: 'Hire employees', D: 'Register with SARS' }, correct_answer: 'B', explanation: 'Business plan outlines goals', hints: ['Think planning'] } },
  ...
  { subject: 'Life Orientation', type: 'mcq', difficulty: 2, xp_reward: 30, content: { question: 'What is a healthy way to manage stress?', options: { A: 'Ignoring it', B: 'Exercise and talking', C: 'Eating junk food', D: 'Sleeping less' }, correct_answer: 'B', explanation: 'Exercise helps manage stress', hints: ['Think physical health'] } },
  { subject: 'English Home Language', type: 'mcq', difficulty: 2, xp_reward: 30, content: { question: 'Identify: The wind whispered secrets', options: { A: 'Simile', B: 'Metaphor', C: 'Personification', D: 'Hyperbole' }, correct_answer: 'C', explanation: 'Personification gives human traits', hints: ['Wind cannot whisper'] } },
...
export default async function handler(req: Request) {
  if (req.method === 'GET') {
    ...
    const userSubjects = subjectsParam ? subjectsParam.split(',').map(s => s.trim().toLowerCase()) : [];

    let challenges;
    if (userSubjects.length > 0) {
      challenges = CHALLENGES.filter(ch => userSubjects.some(us => ch.subject.toLowerCase().includes(us) || us.includes(ch.subject.toLowerCase()))).map((ch, i) => ({ ...ch, id: 'dc_' + d + '_' + i, date: d }));
    } else {
      challenges = CHALLENGES.map((ch, i) => ({ ...ch, id: 'dc_' + d + '_' + i, date: d }));
    }

    const result = { challenges, next_reset: new Date(new Date(d).getTime() + 86400000).toISOString() };
